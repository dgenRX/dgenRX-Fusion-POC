...
public class PokerGameManager : NetworkBehaviour
{
    ...
    [Networked] public int LastRaiserIndex { get; private set; } = -1; // Tracks who last raised/bet (for betting round completion)
    ...
    private Dictionary<int, bool> _playersActedThisRound = new Dictionary<int, bool>(); // Track if each player has acted this betting round
    ...
    public void ProcessPlayerAction(PlayerRef playerRef, PokerPlayer.PlayerAction action, int betAmount = 0)
    {
        ...
        _playersActedThisRound[playerIndex] = true;

        // Check if betting round is complete BEFORE advancing
        // A betting round ends when all bets are matched AND action has returned to last raiser
        ...
        if (allBetsMatched)
        {
            // All bets are matched - check if we've returned to the last raiser
            if (LastRaiserIndex >= 0 && LastRaiserIndex < _players.Count)
            {
                // If current player (who just acted) is the last raiser, round is complete
            ...
            {
                // No one raised - check if all players have acted
                int activePlayerCount = 0;
                int actedPlayerCount = 0;
                for (int i = 0; i < _players.Count; i++)
                {
                    if (!_players[i].IsFolded)
                    {
                        activePlayerCount++;
                        if (_playersActedThisRound.ContainsKey(i) && _playersActedThisRound[i])
                        {
                            actedPlayerCount++;
                        }
                    }
                }
                // If all active players have acted, round is complete
        ...
        // If betting round is complete, advance phase (don't advance to next player)
    ...
    private void AdvanceGamePhase()
    {
        ...
        CurrentBet = 0;
        LastRaiserIndex = -1; // Reset last raiser
        _playersActedThisRound.Clear(); // Reset action tracking for new betting round
